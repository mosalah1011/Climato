...
void init_Accelerometre() {
  ...
  Serial.println("> Accéléromètre initialisé correctement.");
}
...
void init_Anemometre(void) {
  ...
  attachInterrupt(digitalPinToInterrupt(BROCHE_ANEMOMETRE), IRQ_anemometre, RISING);
  Serial.println("> Anémomètre initialisé correctement.");
}
...
//=======================================================================================
//Lecture de la tension d'entrée de la girouette pour ensuite la convertir en orientation
//=======================================================================================
void lecture_Girouette() {
  RawData_girouette = analogRead(BROCHE_GIROUETTE);

  //Applique un polynome de degré 4 pour corriger la "non-linéarité" du ADC et obtenir la tension de la girouette en Volts
  if (RawData_girouette < 1 || RawData_girouette > 4095) {
    Vin_girouette = 0;
  }
  else {
    Vin_girouette = coeff_A * pow(RawData_girouette, 4) + coeff_B * pow(RawData_girouette, 3) + coeff_C * pow(RawData_girouette, 2) + coeff_D * RawData_girouette + coeff_E;
  }
  movA_Girouette.add(Vin_girouette);
  moyenneVin_Girouette = movA_Girouette.get();

  //REVOIR LA SECTION DE CODE CI-DESSOUS !!!! FAIT CRASHER LE ESP32 !!!!!!!!!!!
  //  for (int i = 0 ; i < 17 ; i++) {
  //    //Compare la tension d'entrée à une direction afin d'y connaître son orientation et son degrée (voir section "Girouette" dans l'onglet "station.h"
  //    if(String(moyenneVin_Girouette) <= String((Direction[i] + 0.01)) && String(moyenneVin_Girouette) >= String((Direction[i] - 0.01))) {
  //      orientation = pole[i];
  //      degree = Degree[i];
  //    }
  //  }
...
void lecture_Humidite() {
  ...
  movA_Humidite.add(h);
  //humidite = (1 - alpha) * humidite + alpha * h; //moyenne glissante
  humidite = movA_Humidite.get();
}
...
void init_Luminosite() {
  if (light.begin()) {
    Serial.println("> Impossible de communiquer avec le module GY-49, verifiez le cablage !");
    while (1);
  }
  else {
...
void lecture_Luminosite() {
  ...
  //lux = (1 - alpha) * lux + alpha * L; //moyenne glissante
  movA_Luminosite.add(L);
  lux = movA_Luminosite.get();
}
...
void lecture_Pression() {
  lecture_Pression_OK();
  float p = bmp.pressure / 1000.0;
  //pression = (1 - alpha) * pression + alpha * p; //moyenne glissante
  movA_Pression.add(p);
  pression = movA_Pression.get();
}

//=======================================
//Vérifie si le BMP388 mesure corretement
//=======================================
void lecture_Pression_OK() {
  if (! bmp.performReading()) {
    Serial.println("N'a pas réussi à lire");
    return;
  }
}
...
void lecture_Temperature() {
  ...
  //temperature = (1 - alpha) * temperature + alpha * t; //moyenne glissante
  movA_Temperature.add(t);
  temperature = movA_Temperature.get();
}
...
void postServeur(void) {
  ...
  doc_2["sensor"] = "Vitesse";
  doc_2["value"] = String(vitesse, 1);
...
void valCapteursPortSerie() {
  ...
  Serial.println("> Vitesse du vent................................... " + String(vitesse) + " Km/h");
